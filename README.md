  ...
  * **Owlshift** (`--owlshift`):
    ...
    * For a given input value $v$:
      ...
      2. The mantissa is scaled based on the target FP8 format's mantissa bits (MANTISSA_BITS). For normal numbers:
        $$ m_{\text{scaled}} = \left( \frac{|v|}{2^{e - \text{EXPONENT_BIAS}}} - 1.0 \right) \times 2^{\text{MANTISSA_BITS}} $$
      ...
      4. The stochastically rounded mantissa $m_{\text{final}} = m_{\text{stoch}} / 2^{\text{MANTISSA_BITS}}$ is used to reconstruct the number.
